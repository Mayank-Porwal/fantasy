...
function* getPlayerStats(action: actionTypes.PlayerStatsInterface) {
  try {
    const response: ResponseGenerator = yield call(_request, {
      url: `${API_URLS.FETCH_PLAYER_STATS}?player_id=${action.payload.player_id}&n=${action.payload.n}`,
      method: REQUEST_TYPE.GET,
      headers: HTTPS_HEADERS,
    })
    if (response.status && response.status >= 200 && response.status <= 299) {
      yield put(actionTypes.fetchPlayerStatsSuccess(response?.data))
    } else {
      yield put(
        actionTypes.fetchPlayerStatsFailure(
          response?.data.message ? response?.data.message : MESSAGES.SOMETHING_WENT_WRONG,
        ),
      )
    }
  } catch (e) {
    yield put(actionTypes.fetchPlayerStatsFailure(MESSAGES.SOMETHING_WENT_WRONG))
    console.error(e)
  }
}
...
function* createTeamSaga() {
  yield all([
    ...
    takeLatest(actionTypes.CREATE_TEAM_ACTIONS.FETCH_PLAYER_STATS, getPlayerStats),
  ])
}
